...
function (_, module) {
  ...
  module.controller('EndpointConfCtrl', function(
    ...
    $scope.getCollectors = function() {
      return backendSrv.get('api/plugin-proxy/worldping/api/collectors').then(function(collectors) {
    ...
    $scope.getMonitorTypes = function() {
      return backendSrv.get('api/plugin-proxy/worldping/api/monitor_types').then(function(types) {
        var typesMap = {};
    ...
    $scope.getEndpoints = function() {
      var promise = backendSrv.get('api/plugin-proxy/worldping/api/endpoints');
    ...
    $scope.getEndpoint = function(idString) {
      ...
      _.forEach($scope.endpoints, function(endpoint) {
        if (endpoint.id === id) {
          ...
          backendSrv.get('api/plugin-proxy/worldping/api/monitors?endpoint_id='+id).then(function(monitors) {
    ...
    $scope.setEndpoint = function(id) {
      $location.path('worldping/endpoints/edit/'+id);
    };

    $scope.remove = function(endpoint) {
      backendSrv.delete('api/plugin-proxy/worldping/api/endpoints/' + endpoint.id).then(function() {
        $location.path('worldping/endpoints');
      });
    };

    $scope.removeMonitor = function(mon) {
      var type = $scope.monitor_types[mon.monitor_type_id];
      backendSrv.delete('api/plugin-proxy/worldping/api/monitors/' + mon.id).then(function() {
    ...
    $scope.updateEndpoint = function() {
      ...
      backendSrv.post('api/plugin-proxy/worldping/api/endpoints', $scope.endpoint);
    };
    ...
    $scope.save = function(location) {
      ...
      promises.push(backendSrv.post('api/plugin-proxy/worldping/api/endpoints', $scope.endpoint));
      $q.all(promises).then(function() {
        if (location) {
    ...
    $scope.addMonitor = function(monitor) {
      monitor.endpoint_id = $scope.endpoint.id;
      return backendSrv.put('api/plugin-proxy/worldping/api/monitors', monitor, true).then(function(resp) {
    ...
    $scope.updateMonitor = function(monitor) {
      ...
      return backendSrv.post('api/plugin-proxy/worldping/api/monitors', monitor, true).then(function() {
        var type = $scope.monitor_types[monitor.monitor_type_id];
    ...
    $scope.discover = function(endpoint) {
      ...
      $scope.discoveryError = false;
      backendSrv.get('api/plugin-proxy/worldping/api/endpoints/discover', endpoint).then(function(resp) {
        if (!$scope.showConfig) {
    ...
    $scope.addEndpoint = function() {
      ...
      backendSrv.put('api/plugin-proxy/worldping/api/endpoints', payload).then(function(resp) {
        $scope.endpoint = resp;
        $scope.ignoreChanges = true;
        alertSrv.set("endpoint added", '', 'success', 3000);
        $location.path("worldping/endpoints/summary/"+resp.id);
      });
    };
