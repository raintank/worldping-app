...
function (_, module) {
  ...
  module.controller('CollectorConfCtrl', function($scope, $q, $location, $timeout, $routeParams, $http, backendSrv, contextSrv) {
    ...
    $scope.remove = function(collector) {
      backendSrv.delete('api/plugin-proxy/worldping/api/collectors/' + collector.id).then(function() {
        $scope.getCollectors();
      });
    };

    $scope.save = function() {
      return backendSrv.post('api/plugin-proxy/worldping/api/collectors', $scope.collector);
    };
    ...
    $scope.add = function() {
      backendSrv.put('api/plugin-proxy/worldping/api/collectors', $scope.collector)
        .then(function(resp) {
          $scope.collector = resp;
          $location.path('/worldping/probes/init/'+resp.id);
        });
    };
    ...
    $scope.checkIfOnline = function() {
      ...
      backendSrv.get('api/plugin-proxy/worldping/api/collectors/'+$scope.collector.id).then(function(res) {
        $scope.collector = res;
        if (!res.online) {
